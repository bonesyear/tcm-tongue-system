...
### #13 产品决策定案（2026-08-25，用户确认）

**决策：雷达图不加荣枯轴（A）；body_luster 判读降级——仅判"干燥/润泽"，不判"神气/荣枯"。**

依据：LLM 通过照片判荣枯不可靠——①光泽是光照产物（同舌顺光/闪光灯观感完全不同，参考舌底闪光灯坑）；②神采需动态观察，静态照片无法判断；③面色光线误判同款问题。荣枯预后价值高但数据源不可信，不给不可靠数据建展示位。
