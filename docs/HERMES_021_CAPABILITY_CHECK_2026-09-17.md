...
**温度真相与实验 1（2026-09-18 补记）**：查 DashScope 官方文档（`qwen-api-via-dashscope`）确认——qwen3.8-max（思考模式）**视觉理解的 temperature 默认为 0.6，0.6 以下的值会被服务端默认改为 0.6**（取值范围 `[0, 2)`）。即本节表格与上文中所有「temperature=0」条件的实际生效值均为 0.6，**与不传参等价**：上表 B 组 vs A 组实为 0.6 vs 默认 0.6，「temp=0 不消除抖动」并不是一次有效干预的失败，而是同一温度下的基线观测。**实验 1（同日，同图交叉交替 temp=0 / temp=1.0 各 10 次，共 20 次）**：9 个字段众数两组全一致；评分层仅 temp=0 组齿痕 3/10、厚薄 2/10 不稳（temp=1.0 组全稳），双侧 Fisher p≈0.21/0.47 均不显著——**0.6 与 1.0 无可辨差异**（温度效应被结构化输出的尖峰分布淹没）。**结论**：该端点视觉理解的随机性下限即 0.6，**温度不是抖动的解药**；点刺 10/10 判有**与采样温度无关**，主源是判据错位 + 服务端不确定性。`scripts/vision_client.py` payload 已显式写 `0.6` 如实反映生效值（准确表述：0.6 以下被钳制到 0.6、0 与默认等价；**不是**「temperature 无效」——0.6 以上参数仍生效，只是对该任务效应微小）。
