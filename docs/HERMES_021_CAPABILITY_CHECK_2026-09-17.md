...
### 8.5 评分词表缺口（本次实测发现，待用户签认）

`src/scoring.py` 的 `TONGUE_BODY_SIZE_MAP = {"适中": 0, "胖大": 7, "瘦小": 6, "肿胀": 9}` **未收录「偏胖」**（临床常用表述）→ 本次档案 `body_size: "偏胖"` 落空得 0 分，属**漏判**。

同类待查：`偏瘦`、`胖嫩`、`瘦薄` 等中间态表述。**新增词条需用户签认分值**（沿用轮次 3 的流程）。
