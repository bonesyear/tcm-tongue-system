...
- ✅ **轮次 2 完成**（2026-09-17，commit `ef2f40a`）：辨证层 #5/#8/#9/#10/#12/#16 + 方案 A 项 2~11 + 新增 #19（`_match_score` 平局取最高分）→ **198 passed**（191+7）｜8 项实测逐条通过｜ruff `F,E722` 全过｜独立 diff 审查通过
  ...
  - **端到端回归**：C 档案样本舌诊分 **5.5 → 6.0**（`coating_moisture`=7 生效，湿盛信号不再被掩盖）✅
...
**④ 步骤**：record.py → validator → 模板/文档 → 测试（新 fixture + 检测优先级/别名/C 档案样本值三条断言/inquiry 扁平计数/pattern_update/端到端 validator）
...
1. **同长度关键词取首词 → 湿盛信号被掩盖（评分低估）**
   ...
   - 影响：**湿盛（滑=7）系统性被正常（润=0）掩盖**——对湿盛型舌象（苔偏滑）是关键信号
...
2. **9 个舌部指标不参与评分（既有设计缺项）** ⚠️ 原稿误列 `coating_thickness`（K3 实证纠正，见下）
   ...
   - 影响：**剥落苔（剥落类舌象随访中的核心观察点）在评分与周报趋势中完全不体现**
...
|---|---|---|
| `coating_peeling` | `{"剥落":6,"剥脱":6,"花剥":6,"地图舌":7,"镜面":9}` | **最高优先**（剥落苔随访场景的核心观察点）；结构性形态，静态照片完全可判 |
...
### 轮次 3 执行结果（2026-09-17 完成，commit `cd3250b`）
...
### 轮次 2.5 执行结果（2026-09-17 完成，commit `22ffdc6`）
...
## 轮次 4：评分口径多维输出（2026-09-17 完成，commit `6fdf28f`）
...
## 轮次 5：周报无观测语义收尾（2026-09-17 完成，commit `bb22bf5`）
...
## 轮次 7：舌体胖瘦词表补漏（2026-09-18）

**缺口发现**：2026-09-18 随访档案 `body_size = "偏胖"` 未命中 `TONGUE_BODY_SIZE_MAP`（原仅 适中/胖大/瘦小/肿胀）→ 实测漏判为 0 分。

**只读分析（K3）→ 独立复验**：K3 报告定位词表缺口并给出分值建议；独立复验实测平局红线——`"偏胖大"` 文本中「胖大」(7) 与「偏胖」同长度平局、取分值高者，故**偏胖必须 ≤ 7**，若 ≥ 8 会翻转取偏胖，导致 `tests/test_scoring.py:332` 与周报 fixture（mean 5.5 / max 7.0 / n 4）期望值全部变红。

**用户签认分值**：稍胖 3、略瘦 3、偏胖 4、偏瘦 4（既有 适中 0 / 胖大 7 / 瘦小 6 / 肿胀 9 不动）。

**复合词决策**：「胖嫩/瘦薄」**不收入词表**——它们混入老嫩维度，与「评分层只收静态照片可客观判读指标」的统一原则冲突（荣枯/老嫩已整体降级到辨证层）；老嫩走 prompt 规范化或辨证层，不得隐式计入胖瘦轴。配套在 `scripts/vision_client.py` 舌面 prompt 增加胖瘦输出规范化：只填规范词（正常/适中、偏胖、胖大、偏瘦、瘦小，必要时肿胀），复合描述只取胖瘦成分（胖嫩→偏胖、瘦薄→偏瘦）；「舌质润燥」键名约定与「不判舌神/荣枯」「区分正中沟与病理裂纹」要求原样保留。

**回归结论（实跑）**：

- **235 passed**（232+3，新增 `test_body_size_new_intermediate_terms` / `test_body_size_pianpangda_not_downgraded` / `test_body_size_negation_guard` 共 7 断言）｜ruff `F,E722` 全过
- **新词实测 ✅**：偏胖→4 / 偏瘦→4 / 稍胖→3 / 略瘦→3 / **偏胖大→7（平局不降级）** / 无明显偏胖→0 / 偏胖不明显→0
- **端到端 ✅**：`records/daily/2026-09-18_analysis.json` 舌诊综合偏离度 **0.0 → 4.0**（body_size 命中偏胖=4，其余指标全 0）
- **回归 ✅**：fixture `2026-06-25_analysis.json` 周报舌诊 detail 仍为 **mean 5.5 / max 7.0 / n 4**，既有测试期望值一律未动

---

## 轮次 8：跨维度词表补漏 + 档案数据约定（2026-09-18）

**复审发现**：

1. **kimi 错误 1 修正**：前轮复审曾建议舌质「淡紫」给 7 分——与「淡白: 7（偏寒）」同档过高；舌质淡紫 = 血瘀轻/寒凝，属轻中度异常，用户定 **6**。
2. **淡紫同名异义**：`TONGUE_BODY_COLOR_MAP` 的「淡紫」（舌质 = 异常 6）与 `SUBLINGUAL_COLOR_MAP` 的「淡紫: 0」（舌下浅蓝紫 = 生理性正常）同名异义，两字段不可混淆——两表独立评分无串扰（实测隔离 ✅），风险在档案录入侧字段张冠李戴，故由 validator 告警兜底。
3. **8/12 档案不可达的事实**：12 份历史档案中 8 份解析为空（混合形状/键名漂移），评分词表变化对它们无影响——这是「旧档案不修」决策的事实基础。
4. **08-02 假阳性**：`palm_color` 值「红 (豆包2.1, 非暗红)」中判读注记被关键词评分误命中，实测假阳性 4 分——评分类指标的值必须只写当前状态纯描述。

**用户决策（2026-09-18 签认）**：

- **旧档案不追溯**：「老旧档案数据不重要，历史不追溯」——8 份解析为空档案与 08-02 假阳性均不修。
- **P1 词表落地**（6 项）：舌质 淡紫:6；苔厚 薄:0 / 略厚:3 / 稍厚:3 / 偏厚:4（不收缩裸「厚」）；苔腻 微腻:2（硬约束 ≤ 稍腻 3）；唇色 淡红:0+红:3（成对）；唇润燥 润:0 / 稍干:2 / 偏干:2 / 干燥:2；掌色 淡红:0+红:3（成对）。
- **明确不收**：P2 项（偏腻、明显齿痕、血丝、略枯、nail_color/耳基线盾牌词）；偏暗/淡暗、偏枯、甲床淡紫（已知 K3 误报语料）、裸「轻度/明显」、「皮肤淡黄」、**任何形式的裸「正常」**（实测长度压制：「正常:0」+「红:3」→「正常偏红」误判 0）。
- **未来的档案数据不会乱即可**：validator 增加告警级校验（非阻断）。

**落地清单**：

1. `src/scoring.py`：上述 6 项词表全部落地，注释载明同名异义、盾牌词（同「红润:0」压「红:7」先例）、「薄」显式基线且不收缩裸「厚」的理由。
2. `src/scoring.py` **额外判断（明确说明）**：lip_color / palm_color 各补「红润: 0」盾牌词——裸「红:3」入表后，纯文本路径下正常描述「红润有光泽」会被跨字段误命中（2 个既有测试变红）；「红润:0」是 TONGUE_BODY_COLOR_MAP 已签认先例的同型处置，取最小改动。
3. `scripts/input_validator.py`：新增 `_validate_scoring_value_hygiene`（校验步骤 6，**纯告警**，不改通过/失败判定）——① body_color 与 sublingual_color 同含「淡紫」→ 告警字段归属；② 任一评分类指标值含判读注记特征（括号内出现 豆包/Qwen/K3/用户确认/误判/非）→ 告警移入 notes/lessons。评分类指标清单直接取 `DIMENSION_RULES`（数据驱动，不手抄）。
4. `tests/test_scoring.py`：新增 7 个测试函数（淡紫 6 + 同名异义隔离 / 苔厚新词 / 微腻+硬约束 / 唇色盾牌词成对+反例锁死 / 唇润燥新词 / 掌色盾牌词成对+反例锁死 / 裸「正常」词条永不入表）。
5. `tests/test_scoring.py` **2 个既有测试入参结构化（期望值不变）**：`test_no_false_match_red_ruyang` 与 `test_non_tongue_score_mean_is_always_float` 由纯文本入参改为结构化 dict 入参——lip_color 裸「红:3」使纯文本路径必然跨字段 double-score（score() docstring 早已载明该局限，生产路径 Record.get_observation 本就是结构化）；同轮次 3 `test_tongue_score_sparse_not_diluted` 改结构化的先例，断言分值与 float 类型检查原样保留。

**回归结论（实跑）**：

- **242 passed**（235+7）｜ruff `F,E722` 全过
- **新词实测 ✅**：淡紫→6 / 稍厚→3 / 偏厚→4 / 微腻→2 / 略厚→3 / 薄→0 / 唇 淡红→0+红→3 / 唇 偏干→2 / 掌 淡红→0+红→3
- **同名异义隔离 ✅**：同一观测 dict 中 body_color 淡紫→6、sublingual_color 淡紫→0，两表独立
- **长度压制反例 ✅**：全部评分表确认无裸「正常」词条
- **validator 新告警实测 ✅**：两处淡紫同现 → 告警；「红 (豆包2.1, 非暗红)」→ 告警；干净样例 → 0 告警；三者退出码均 0（告警不阻断）
- **fixture 回归 ✅**：`2026-06-25_analysis.json` 头面诊 detail **5.0/5.0/1 → 7.0/5.0/2**（唇润燥「偏干」命中新词，口径变化的预期效果）；舌诊仍 **5.5/7.0/4**，其余维度不变；`test_weekly_report.py` 无精确断言变红，未改动


---

## 轮次 9：复审问题清单修复（2026-09-18，用户批准五项）

**来源**：轮次 8 后复审建立的问题清单，用户批准修复 ① ② ③ ⑤ + 盾牌词补齐（仅 `lip_color` 淡粉红；P2 的三个基线盾牌词 nail_color 淡红/淡粉、ear.color 淡红、face_luster 有光泽实测非必需，不做——这些表内无 1 字异常词，无从误命中）。

### ⑤-0 盾牌词：`lip_color` += 「淡粉红: 0」

实测 `lip_color` 表 `{淡红:0, 红润:0, 红:3, 紫暗:3, 青紫:3}` 下探针「淡粉红」误命中裸「红」得 3 分。「淡粉红: 0」以最长匹配压过，注释注明盾牌词用途（同轮次 8「淡红/红润」先例）。

### ① 漂移键静默丢弃 → 告警升级（`src/record.py`）

实测 `records/daily/2026-08-02_analysis.json` 的 `tongue` 下 10 键中 4 个漂移键（`body_shape`/`teeth_marks`/`crack`/`sublingual_veins`）被静默丢弃——`sublingual_veins` 的值是「异常—中度血瘀」铁证级数据。根因：`_warn_if_mixed_shape` 旧口径只在维度节点**全部**解析不到时才告警（6/10 部分解析成功不触发）。

**升级**：「存在未识别键即告警」并列出被丢弃键名；仍是 warning（非阻断），不改 `get_observation` 返回语义，不改覆盖度/评分，沿用 `_warned_mixed_shape` 每维度去重；未识别键全为空值占位时不触发。

### ② 占位文本约定：「正常（用户声明，未拍摄）」→「未拍摄（用户声明正常）」

旧文本声明「未拍摄」却以「正常」打头，与 `templates/adaptive_analysis_prompt.md`「绝不编造观察」有张力，且既有注记判据（括号内模型名）抓不到它。**约定：未拍摄的维度一律写「未拍摄（用户声明正常）」**——事实（未拍摄）在前，用户声明降级为括号注记。

- `scripts/input_validator.py` 评分值卫生校验新增第 3 条（纯告警）：值以「正常」开头且含「未拍」→ 告警语义矛盾并建议新写法。判据保守（「正常」打头 + 含「未拍」），推荐写法「未拍摄（用户声明正常）」不触发。
- 本地档案 `records/daily/2026-08-28_analysis.json` 与 `records/daily/2026-09-18_analysis.json`（gitignored）共 26 处占位文本已改写；复跑 validator：无严重错误、覆盖度仍 6/6、评分不变（该文本不含任何评分关键词；`is_covered` 值非空即覆盖不受影响）。

### ③ `palm_temp` 从评分表降级（`src/scoring.py`）

`palm_temp`（厥冷8/不温4/凉4/温0）需触诊/问诊，静态照片判不了——违反统一原则「评分层只收静态照片可客观判读的指标」（本文档轮次 3 节）；且为 6 维度中唯一零语料指标（从无档案赋值）。**从 `DIMENSION_RULES[HAND]` 移除**（HAND 指标 5→4），降级到辨证层（掌温由问诊采集，见 `docs/user-guide.md`）；原词表以注释留存备查。观测层（`record.py` 指标路径）保留——问诊文本仍可进辨证 prompt。fixture 手诊 detail 不变（4.0/2.0/2 = palm_color 2 + nail_color 2，本就不含 palm_temp）。

### ⑤ P2 词条 3 项 + 齿痕走 prompt 规范化

轮次 8「明确不收」的 P2 项本轮部分翻案（用户签认 2026-09-18）：

- `TONGUE_COATING_GREASY_MAP` += 「偏腻: 4」（原命中裸「腻」得 5 分偏重；理顺 微腻2 < 稍腻3 < 偏腻4 < 腻5）
...
## 轮次 11：点刺「少量」漏判修复——词表兜底 + prompt 规范化（2026-09-18，方案 C）

> 编号说明：用户任务书称本轮为「方案 C」；本文档轮次 10 已被「detail 字段改名」占用，故本轮记为**轮次 11**（沿用轮次 10 的编号说明做法）。

**问题**（KNOWN_ISSUES §3 #6）：06-25 真实档案的舌面点刺值为「少量」（`observations.tongue.body.prickles`）→ 未命中 `TONGUE_PRICKLES_MAP`（仅 `{点刺:5, 芒刺:6}`）→ MISS 落 0，该日点刺信号丢失。

**方案（用户认可 2026-09-18，治本 + 兜底两条都做）**：

- **A（兜底）**：`TONGUE_PRICKLES_MAP` += 「少量: 2」。2 分保守（远低于点刺 5/芒刺 6），符合该表「分辨率敏感 → 分值保守」的既定口径（`src/scoring.py` 表头注释）。既有 `点刺: 5` / `芒刺: 6` 不动。
- **B（治本，根本解法）**：`scripts/vision_client.py` 舌面 prompt 增加点刺字段规范化——「点刺只填规范词：无/点刺/芒刺（不填"少量""散在""不明显""散在红点刺"等描述性短语）」，与轮次 9 齿痕规范化句对称；既有 5 条 prompt 约定（「舌质润燥」键名单值、胖瘦规范词+复合词取胖瘦成分、齿痕规范词、正中沟/病理裂纹区分、不判舌神/荣枯）逐项保留。

**实测复核（词表补法零风险确认）**：

- `少量`→2；`散在少量`→2；`少量点刺`/`点刺少量`→**5**（"少量"2 字 = "点刺"2 字，同长平局取高，方向正确，无长度压制）
- 否定守卫不受影响：`无点刺`→0、`点刺不明显`→0
- 无跨字段串扰：其余 38 个指标表均未收「少量」，且 `prickles` 表独立

**落地清单**：

1. `src/scoring.py`：`TONGUE_PRICKLES_MAP` += 「少量: 2」（注释载明保守档理由、同长平局方向、否定守卫与隔离复核结论）。
2. `scripts/vision_client.py`：舌面 prompt 加点刺规范化句（齿痕句之后、正中沟句之前）。
3. `tests/test_scoring.py`：新增 `test_prickles_shaoliang_conservative`（6 断言：少量→2 / 散在少量→2 / 少量点刺→5 / 点刺少量→5 / 无点刺→0 / 点刺不明显→0）；既有 `test_prickles_scoring`（点刺 5 / 芒刺 6 / 无点刺 0）原样未动。
4. `tests/test_vision_client.py`：既有 prompt 断言仅钉「舌质润燥」键名（test ⑪），本轮未改该键名，无需同步；齿痕规范化句上轮亦未加断言，本轮对齐不新增（最小改动）。
5. 文档：KNOWN_ISSUES #6 标记已解决；CHANGELOG 追加 v1.4.2。

**回归结论（实跑，2026-09-18）**：

- **251 passed**（250+1 新测试函数）｜ruff `F,E722` 全过
- **fixture 回归 ✅**：`tests/fixtures/2026-06-25_analysis.json` 的 `prickles` 值为「舌尖散在点刺」（非「少量」）→ 仍命中点刺 5 分，周报舌诊 detail 期望 `{'score': 5.5, 'max': 7.0, 'n': 4}` **不变**，`test_weekly_report.py` 无断言变红
- **真实档案影响（只读，未修改）**：`records/daily/2026-06-25_analysis.json`（gitignored）的 `prickles=少量` 得分 0 → **2**，下次生成 W26 周报时该日点刺信号恢复（轻量 2 分，符合保守口径）
