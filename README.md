...
- **本程序本身不收集、不上传、不传输任何用户数据**——无遥测、无统计上报、无回传机制。
- 它是一个**纯本地数据模型库**：照片、记录（`records/`）、图表（`charts/`）全部只存在你自己的机器上。
- 唯一的网络调用发生在**你自配置的外层 Agent**（如视觉模型 API、LLM API），须使用你自己的 API key；本库自身零网络请求。
- 默认的 OSS 备份脚本**已排除全部健康数据**（`records/`、`charts/`）与版权全文——需你主动配置才会启用。
- 健康数据属敏感个人信息，请自行妥善保管本地目录与备份权限。
